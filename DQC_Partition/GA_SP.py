...
def spectral_partitioning(gate_list,num_partitions):
    ...
    # Compute the first k eigenvectors of the normalized Laplacian matrix
    eigenvalues,eigenvectors = np.linalg.eig(l_norm)
    idx = np.argsort(eigenvalues)[1:num_partitions]
    eigenvectors = eigenvectors[:,idx]

    # Cluster the eigenvectors using KMeans
    kmeans = KMeans(n_clusters=num_partitions,random_state=0,n_init=10)
...
def visualize_teleportation_cost():
    ...
    k2_telep_cost = [12,6,4,8,60,4,4,2,2]
    k3_telep_cost = [26,18,6,24,120,6,8,6,4]
    k4_telep_cost = [48,34,12,34,162,14,14,8,6]
    ...
    plt.title('GA und SP Teleportationskosten')
...
if __name__ == '__main__':
    ...
    input_filename = 'C:/Users/Teodor/QC_GA_Partition/qasm/qft16_basic.qasm'
